...
const getStaticProps = async context => {
  const response = await fetch(`${_config__WEBPACK_IMPORTED_MODULE_3__/* .server */ .f}/api/destinations/czech-republic/${context.params.id}`);
...
const getStaticPaths = async () => {
  const response = await fetch(`${_config__WEBPACK_IMPORTED_MODULE_3__/* .server */ .f}/api/destinations/czech-republic`);
  const data = await response.json();
...
var __webpack_exports__ = __webpack_require__.X(0, [2821,5675,1664,6893,1677,7061,9415,6520], function() { return __webpack_exec__(397); });
